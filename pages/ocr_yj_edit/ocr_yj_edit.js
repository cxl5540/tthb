...
Page({
        ...
        success:res=>{
          if(res.code==200){
            ...
            this.setData({
              ...
              'formData.hj':this.formatDecimal(invoice_verify_info.AmountInFiguers),
              year:res.data.year,
              id:res.data.id
            })
            if(!formData.xfmc){
  ...
  inputWacth(e){
    ...
    if(formData.je&&item=='je'){
      if(formData.sl){
        formData['se']=formatDecimal((Number(formData.je)*Number(formData.sl)/100),2)
      }else{
        formData['se']=formatDecimal((Number(formData.je)*0).toFixed(2),2)
      } 
      formData['hj']=formatDecimal((Number(formData.je)+Number(formData.se)),2)
    }
    this.setData({
    formData
    });
      ...
      success:res=>{
        if(res.code==200){        
          this.setData({
            ...
            date:util.getdate(res.data.invoice_info.invoice_date)
          })
